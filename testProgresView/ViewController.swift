...
class ViewController: UIViewController {
    ...
    @IBAction func onTest(_ sender: UIButton) {
        ...
        var key = StorageDomain.database.user.firstLogin
        if let dictToSave = dictForValue(45, reversible: key) {
            print(dictToSave)
            dict2store = append(dict: dictToSave, to: dict2store)
        }
        
        key = StorageDomain.database.user.lastLogin
        if let dictToSave = dictForValue("yesterday", reversible: key) {
            print(dictToSave)
            dict2store = append(dict: dictToSave, to: dict2store)
        }
        
        key = StorageDomain.database.server.sessionTimeout
        if let dictToSave = dictForValue([1, 3, 7, 11, 13], reversible: key) {
            print(dictToSave)
            dict2store = append(dict: dictToSave, to: dict2store)
        }
        
        key = StorageDomain.database.user.oneLimeLogic.infoPopup
        if let dictToSave = dictForValue(true, reversible: key) {
            print(dictToSave)
            dict2store = append(dict: dictToSave, to: dict2store)
        }
    }
    ...
    @IBAction func testStore(_ sender: Any) {
        let key = StorageDomain.database.server.sessionTimeout
        FileStorage.global[key] = 123
    }
