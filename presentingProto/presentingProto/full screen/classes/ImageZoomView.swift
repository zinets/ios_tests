...
// TODO: я подозреваю, что этого хлама в каждом проекте полно своего; по хорошему он нужен только для фулскрина (и он используется "дефолтным" аниматором, так что..), так что пусть будет в фулскрине - а в остальных локациях фйл надо удалить к херам 
...
//            dblTapGesture.isEnabled = true
    ...
    private func scalesForZooming() {
        if let image = image {
            ...
            // ХЗ что за магическое число, оно по факту в некоторых случаях выходит меньше, чем минимум зум, поэтому пока в коммент")
            self.maximumZoomScale = max(scaleToFit, scaleToFill) //1.4
